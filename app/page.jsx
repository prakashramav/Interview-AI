...
export default function Home(){
  return(
    <div className="overflow-x-hidden">
        ...
        <div className="relative z-10 col-span-full lg:col-span-3 flex flex-col items-center justify-center text-center lg:-rotate-2">
          ...
          <div className="relative flex items-center justify-center gap-3 sm:gap-4 mt-8 sm:mt-16">
            <div className="flex">
              {
                AVATARS.map((avatar, index) => (
                  <div key={index} className={`w-8 h-8 rounded-full border-2 border-white overflow-hidden ${index > 0 ? '-ml-2' : ''}`}>
                    <Image
                      src={avatar.src}
                      alt="User avatar"
                      width={32}
            ...
            <p className="text-sm text-stone-500 text-center sm:text-left">
              <strong className="text-stone-400 font-medium">2,400+ engineers</strong>{" "} cracked FAANG interviews via Prept
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-24 px-6">
          {
            LOGOS.map((logo) => (
              <Image                
                ...
                width={120}
                height={60}
                className="mx-4 my-2 grayscale brightness-75 contrast-125 opacity-90 hover:grayscale-0 hover:brightness-100 transition duration-300"
              />
            ))
          }
        </div>
      </section>
      <section className="relatives z-10 py-8 max-w-5xl mx-auto px-6">
      ...
      <section className="relative z-10 pb-24 max-w-4xl mx-auto px-4 sm:px-6">
        <div className="relative border border-amber-400/20 rounded-3xl px-6 sm:px-12 py-16 bg-linear-to-br from-amber-400/5 text-center overflow-hidden">
          
          <StarsBackgroundDemo />

          <h2 className="font-serif relative text-3xl sm:text-4xl md:text-5xl leading-tight tracking-tight mb-4">
            <GrayTitle>Your next interview</GrayTitle>
            <br />
            <GoldTitle>starts here</GoldTitle>
          </h2>

          <p className="relative text-stone-400 font-light text-sm mb-10">
